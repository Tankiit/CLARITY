...
"""
SST-2 Experiments with Diverse Dropout Credal CBM
...
def run_sst2_diverse_dropout_experiment(
    epochs: int = 50,
    batch_size: int = 32,
    lr: float = 2e-5,
    max_samples: int = 1000,
    device: str = 'cuda' if torch.cuda.is_available() else 'cpu',
    save_dir: str = './results_sst2_diverse_dropout'
):
    """
    Run SST-2 experiment with diverse dropout training + uncertainty evaluation
    ...
    print("="*80)
    print("SST-2 DIVERSE DROPOUT CREDAL CBM EXPERIMENT")
    print("="*80)
    ...
    checkpoint_path = os.path.join(save_dir, 'credal_cbm_sst2_diverse_dropout.pt')
    torch.save({
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Run SST-2 Diverse Dropout Credal CBM experiments')
    ...
    parser.add_argument('--save_dir', type=str, default='./results_sst2_diverse_dropout', help='Save directory')

    args = parser.parse_args()

    run_sst2_diverse_dropout_experiment(
        epochs=args.epochs,
